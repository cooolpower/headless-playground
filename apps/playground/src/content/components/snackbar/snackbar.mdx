...
import { SnackbarCss } from '@cooolpower/headless-ui';
...
`Snackbar` 컴포넌트는 내부적으로 다음과 같은 고정 클래스명을 제공합니다.

> [!NOTE]
> **제공되는 내부 클래스:**
>
> - `.hcSnackbar`: 스택바 전체 컨테이너
> - `.hcSnackbarBody`: 메시지 텍스트가 들어가는 본문 영역
> - `.hcSnackbarAction`: 액션 버튼이 배치되는 영역
> - `.hcSnackbarClose`: 닫기 버튼
> - `.hcSnackbarProgressTrack`: 자동 닫힘 진행바의 트랙
> - `.hcSnackbarProgressFill`: 자동 닫힘 진행바의 채워지는 영역
...
export const customSnackbarWrapper = style({});

// 스낵바 본체 디자인 커스텀
globalStyle(`${customSnackbarWrapper} .hcSnackbar`, {
...
// 프로그레스 바 색상 커스텀
globalStyle(`${customSnackbarWrapper} .hcSnackbarProgressFill`, {
  backgroundColor: '#60a5fa',
});

// 액션 버튼 스타일 (내부 버튼이 배정될 경우)
globalStyle(`${customSnackbarWrapper} .hcSnackbarAction button`, {
  color: '#60a5fa',
  fontWeight: '700',
