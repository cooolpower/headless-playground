...
import { SliderCss } from '@cooolpower/headless-ui';
...
`Slider` 컴포넌트는 내부적으로 다음과 같은 고정 클래스명을 제공합니다.

> [!NOTE]
> **제공되는 내부 클래스:**
>
> - `.hcSlider`: 전체 슬라이더 컨테이너
> - `.hcSliderTrack`: 슬라이더의 전체 트랙(레일)
> - `.hcSliderFill`: 현재 값까지 채워진 트랙 영역
> - `.hcSliderHandle`: 드래그 가능한 핸들(Thumb)
> - `.hcSliderTooltip`: 핸들 조작 시 나타나는 값 표시 툴팁
> - `.hcSliderMarkDot`: 특정 눈금에 표시되는 점
> - `.hcSliderMarkLabel`: 눈금 아래 표시되는 텍스트/아이콘 라벨
...
// 트랙과 핸들 기본 스타일 커스텀
globalStyle(`${customSliderWrapper} .hcSliderTrack`, {
  height: '8px',
  backgroundColor: '#e2e8f0',
});

globalStyle(`${customSliderWrapper} .hcSliderFill`, {
  backgroundColor: '#8b5cf6',
});

globalStyle(`${customSliderWrapper} .hcSliderHandle`, {
  width: '20px',
  height: '20px',
...
// 툴팁 디자인 커스텀
globalStyle(`${customSliderWrapper} .hcSliderTooltip`, {
