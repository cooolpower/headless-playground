...
import { PaginationCss } from '@cooolpower/headless-ui';
...
`Pagination` 컴포넌트는 내부적으로 다음과 같은 고정 클래스명을 제공합니다.

> [!NOTE]
> **제공되는 내부 클래스:**
>
> - `.hcPagination`: 전체 페이지네이션 컨테이너
> - `.hcPaginationTotal`: 총 항목 수 표시 영역
> - `.hcPaginationButtons`: 페이지 번호 버튼들을 감싸는 컨테이너
> - `.hcPaginationBtn`: 개별 페이지 번호 및 이전/다음 버튼
> - `.hcPaginationExtras`: 추가 컨트롤 영역
> - `.hcPaginationQuickJumper`: 빠른 페이지 이동 입력 영역
...
export const customPaginationWrapper = style({});

// 개별 페이지 버튼 스타일 커스텀
globalStyle(`${customPaginationWrapper} .hcPaginationBtn`, {
  width: '36px',
  height: '36px',
...
// 활성화된 페이지 버튼 스타일
globalStyle(`${customPaginationWrapper} .hcPaginationBtn[data-active="true"]`, {
...
// 호버 효과
globalStyle(
  `${customPaginationWrapper} .hcPaginationBtn:hover:not([aria-disabled="true"])`,
  {
