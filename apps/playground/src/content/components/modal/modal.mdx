...
import { ModalCss } from '@cooolpower/headless-ui';
...
`Modal` 컴포넌트는 내부적으로 다음과 같은 고정 클래스명을 제공합니다.

> [!NOTE]
> **제공되는 내부 클래스:**
>
> - `.hcModalMask`: 모달 전체를 감싸는 고정 위치 마스크
> - `.hcModalBackdrop`: 배경 흐림/색상을 담당하는 배경 영역
> - `.hcModalWrap`: 모달의 위치를 잡는 래퍼
> - `.hcModal`: 실제 모달 박스
> - `.hcModalHeader`: 헤더 영역
> - `.hcModalTitle`: 제목 텍스트 영역
> - `.hcModalClose`: 닫기 버튼
> - `.hcModalBody`: 콘텐츠가 들어가는 본문 영역
> - `.hcModalFooter`: 하단 버튼 등이 배치되는 푸터 영역
...
export const customModalWrapper = style({});

// 배경 마스크 디자인 커스텀
globalStyle(`${customModalWrapper} .hcModalBackdrop`, {
  backgroundColor: 'rgba(15, 23, 42, 0.9)',
  backdropFilter: 'blur(12px)',
});

// 모달 본체 스타일 커스텀
globalStyle(`${customModalWrapper} .hcModal`, {
  borderRadius: '32px',
...
// 제목 폰트 스타일
globalStyle(`${customModalWrapper} .hcModalTitle`, {
  fontSize: '1.5rem',
  fontWeight: '900',
  textAlign: 'center',
  width: '100%',
...
// 본문 패딩 조정
globalStyle(`${customModalWrapper} .hcModalBody`, {
  padding: '40px',
  textAlign: 'center',
});
```
