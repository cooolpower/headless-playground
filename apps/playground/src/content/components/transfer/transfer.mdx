...
import { TransferCss } from '@cooolpower/headless-ui';
...
```tsx
import type { TransferClassNames } from '@cooolpower/headless-ui';
...
`Transfer` 컴포넌트는 내부적으로 다음과 같은 고정 클래스명을 제공합니다.

> [!NOTE]
> **제공되는 내부 클래스:**
>
> - `.hcTransfer`: 전체 트랜스퍼 컨테이너
> - `.hcTransferList`: 좌/우 개별 리스트 박스
> - `.hcTransferListHeader`: 리스트 헤더 (제목, 카운트)
> - `.hcTransferListTitle`: 리스트 제목
> - `.hcTransferCount`: 항목 개수 표시
> - `.hcTransferListSearch`: 검색바 컨테이너
> - `.hcTransferSearchInput`: 실제 검색 input
> - `.hcTransferListBody`: 항목 리스트 영역
> - `.hcTransferListItem`: 각 행(Row) 아이템
> - `.hcTransferOperations`: 중앙 이동 버튼 그룹
> - `.hcTransferOperationButton`: 이동 버튼 (←, →)
...
export const customTransferWrapper = style({});

// 리스트 박스 디자인 커스텀
globalStyle(`${customTransferWrapper} .hcTransferList`, {
  borderRadius: '12px',
  borderColor: '#e2e8f0',
...
// 헤더 배경색 변경
globalStyle(`${customTransferWrapper} .hcTransferListHeader`, {
  backgroundColor: '#f8fafc',
  borderBottom: '1px solid #e2e8f0',
});

// 이동 버튼 스타일 커스텀
globalStyle(`${customTransferWrapper} .hcTransferOperationButton`, {
...
globalStyle(`${customTransferWrapper} .hcTransferOperationButton:disabled`, {
