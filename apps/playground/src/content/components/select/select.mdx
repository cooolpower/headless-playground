...
import { SelectCss } from '@cooolpower/headless-ui';
...
`Select` 컴포넌트는 내부적으로 다음과 같은 고정 클래스명을 제공합니다.

> [!NOTE]
> **제공되는 내부 클래스:**
>
> - `.hcSelect`: 전체 셀렉트 컨테이너
> - `.hcSelectTrigger`: 클릭 시 드롭다운을 여는 트리거 버튼
> - `.hcSelectValue`: 선택된 값 표시 영역
> - `.hcSelectIcons`: 우측 아이콘 그룹 (화살표, 지우기 버튼 등)
> - `.hcSelectDropdown`: 옵션 목록 드롭다운 패널
> - `.hcSelectSearchWrap`: 검색 입력창 영역
> - `.hcSelectOption`: 개별 선택 항목
> - `.hcSelectOptionLabel`: 옵션의 텍스트 레이블
> - `.hcSelectOptionDescription`: 옵션의 상세 설명
> - `.hcSelectLoading`: 데이터 로딩 중 상태 표시 영역
...
export const customSelectWrapper = style({});

// 트리거 버튼 커스텀
globalStyle(`${customSelectWrapper} .hcSelectTrigger`, {
  borderRadius: '10px',
  borderColor: '#e2e8f0',
  backgroundColor: '#fff',
  height: '42px',
...
// 드롭다운 패널 디자인
globalStyle(`${customSelectWrapper} .hcSelectDropdown`, {
  borderRadius: '12px',
...
// 옵션 호버 및 선택 스타일
globalStyle(`${customSelectWrapper} .hcSelectOption:hover`, {
  backgroundColor: '#f8fafc',
});

globalStyle(`${customSelectWrapper} .hcSelectOption[data-selected="true"]`, {
